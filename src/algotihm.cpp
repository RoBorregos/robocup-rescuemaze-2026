...
void detection(Tile* curr){
    // New Early return if tile is null or already has victim
    ...
    // New Simplified victim handling using switch instead of if-else 
...
void maze::followPath(Stack& path, arrCustom<Tile>& tiles, arrCustom<coord>& tilesMap){
    // New Early return if path is empty (just fot crashes)
...
void maze::dijkstra(coord& start, coord& end, arrCustom<coord>& tilesMap, arrCustom<Tile>& tiles){
    ...
    arrCustom<coord> previousPositions(mapSize, kInvalidPosition);  // Previous tile in optimal path
    ...
    // New Reconstruct path from end to start
...
void maze::dfs(arrCustom<coord>& visitedMap, arrCustom<Tile>& tiles, arrCustom<coord>& tilesMap){
    ...
    while(!unvisited.empty() && !robot.buttonPressed){
        ...
        unvisited.pop();        
        // New Use helper function instead of manual loop
        // Skip if already visited
        if(isVisited(current, visitedMap)) continue;
        
        // Navigate to this position using Dijkstra
        ...
        exploreNeighbors(current, currentTile, unvisited, visitedMap, tiles, tilesMap, false);
    }
    if(unvisited.empty()){ 
...
void maze::run_algs(){ // Initializes data structures and starts DFS exploration
