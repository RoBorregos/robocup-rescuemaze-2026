...
void Tile::setVictim() { this->data_ |= (1 << kVictimBit); }

bool Tile::hasVictim() const { return this->data_ & (1 << kVictimBit); }

void Tile::setObstacle() { this->data_ |= (1 << kObstacleBit); }

bool Tile::hasObstacle() const { return this->data_ & (1 << kObstacleBit); }
...
bool Tile::hasBlackTile() const { return this->data_ & (1 << kBlackTileBit); }

void Tile::setCheckpoint() { this->data_ |= (1 << kCheckpointBit); }

bool Tile::hasCheckpoint() const { return this->data_ & (1 << kCheckpointBit); }

void Tile::addAdjacentTile(const TileDirection direction, Tile *tile,
                           const bool wall, const bool blue) {
  ...
  } else if (this->hasBlackTile() ||
             (tile != nullptr && tile->hasBlackTile())) {
...
void Tile::setPosition(const coord &position) { this->position_ = position; }