...
void motors::ahead(){
   ...
   passObstacle(); //double verification (if obstacle still in the way rotate, else, ignore)
    ...
    float frontDistance=max(vlx[vlxID::frontLeft].getDistance(), vlx[vlxID::frontRight].getDistance());
    float backDistance;
    ...
    if(!encoder){
        ...
        while(frontVlx ? (distance>=targetDistance):(distance<=targetDistance)){
            ...
            if(isRamp()) break;
            //limitCrash();
            distance=(frontVlx ? max(vlx[vlxID::frontLeft].getDistance(), vlx[vlxID::frontRight].getDistance()):vlx[vlxID::back].getDistance());
    ...
    if (vlx[vlxID::left].isWall()&&vlx[vlxID::right].isWall()) {
            ...
            PID_Wheel(error,MotorID::kFrontRight);
            PID_Wheel(error,MotorID::kBackRight);
        }
    */
    stop();
    float angle = bno.getOrientationX();
    if (abs(targetAngle_ - angle)>=10) screenPrint("correcting"); rotate(targetAngle_); screenPrint(" ");
    if(!encoder){
...
bool motors::isWall(uint8_t direction){
    ...
    // screenPrint(String (vlx[vlxID::left].getDistance()));
